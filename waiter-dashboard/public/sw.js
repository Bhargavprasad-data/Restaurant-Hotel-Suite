const CACHE_NAME = 'tasty-suites-waiter-cache-v2';
const STATIC_ASSETS = [
  '/',
  '/index.html',
  '/favicon.ico',
...
self.addEventListener('fetch', (event) => {
  ...
  // Let them go directly to network so app error handling (e.g. AuthContext) works natively
  if (
  ...
  // 4. Handle SPA navigation requests (e.g., /login, /orders, /attendance)
  if (req.mode === 'navigate') {
    event.respondWith(
      fetch(req)
        ...
        .catch(async () => {
          // If offline or network fails, fall back to cached index.html so React Router renders the route
          const cachedIndex = await caches.match('/index.html') || await caches.match('/');
          if (cachedIndex) {
            return cachedIndex;
          }

          // Fallback response guarantees event.respondWith always receives a valid Response
          return new Response(
    ...
    caches.match(req).then((cachedResponse) => {
      ...
      return fetch(req)
        ...
        .catch(async () => {
          ...
          // Return a valid Response object instead of undefined
