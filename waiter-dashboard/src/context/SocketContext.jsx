...
export const SocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      console.log('🔌 Waiter Dashboard connected to WebSocket channel.');
      newSocket.emit('join-room', 'waiter');
    });
    ...
    newSocket.on('user-update', (updatedUser) => {
      ...
      if (currentUser && updatedUser && parseInt(updatedUser.id) === parseInt(currentUser.id)) {
        ...
        setUser(prev => {
          ...
          localStorage.setItem('waiter_user', JSON.stringify(newUser));
          return newUser;
        });
      }
    });
    ...
    // Listen for attendance updates and show toast
